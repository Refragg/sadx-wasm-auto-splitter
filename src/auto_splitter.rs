...
use asr::time::{Duration, Time};
use asr::user_settings::Title;
use asr::{print_message, timer, timer::TimerState, user_settings::Settings, Address, print_limited};
...
pub fn auto_splitter_init(settings: &Settings) {
    print_message(
        "Sonic Adventure DX Auto Splitter - Attached to process, beginning main auto splitter loop",
    )
}

pub fn auto_splitter_start(vars: &GameStatePair, custom_vars: &mut CustomVars, settings: &Settings) -> bool {
    ...
    let story_enter_start = vars.demo_playing.current != 1 && vars.game_status.old == 21 && (vars.game_mode.current != 12 && vars.game_mode.current != 20) && (vars.game_mode.old == 12 || vars.game_mode.old == 20);
...
pub fn auto_splitter_split(vars: &GameStatePair, custom_vars: &mut CustomVars, settings: &Settings) -> bool {
    ...
    if settings.char_split && vars.demo_playing.current != 1 && vars.game_status.old == 21 && (vars.game_mode.current != 12 && vars.game_mode.current != 20) && (vars.game_mode.old == 12 || vars.game_mode.old == 20) {
    ...
    for i in 0..custom_vars.split_mask.len() {
        if custom_vars.split_mask[i] && vars.game_status.current != 21 && (vars.event_bytes.current[i] != vars.event_bytes.old[i]) {
    ...
    // Big ending
    split_if_true!(vars.in_cutscene.changed() && vars.level.current == 29 && vars.current_character.current == 7);
    
    // E-101 split
    split_if_true!(settings.e101_gamma && vars.current_character.current == 6 && vars.e101_flag.increased());

    // MKII split
    split_if_true!(vars.level.current == 25 && vars.mk2_value.current == 255 && settings.boss_25 && vars.boss_health.current == 0 && vars.timer_start.changed_from_to(&1, &0));
    
    // Perfect Chaos split
    split_if_true!(vars.level.current == 19 && settings.boss_19 && vars.boss_health.changed_from_to(&2, &0));
    
    // Zero split
    split_if_true!(vars.level.current == 21 && settings.boss_23 && vars.boss_health.changed_from_to(&2, &1));
    
    // Egg Walker split
    split_if_true!(vars.level.current == 21 && vars.walker_value.current == 9 && settings.boss_21 && vars.boss_health.current == 0 && vars.timer_start.changed_from_to(&1, &0));
    ...
    if custom_vars.boss_splits[vars.level.current] {
        ...
        // Boss beaten checks
        if custom_vars.boss_started && vars.in_cutscene.current == false && vars.in_cutscene.unchanged() && vars.boss_health.current == 0 && vars.game_status.current == 15 && vars.timer_start.changed_from_to(&1, &0) {
    ...
    if vars.level.current != 0 {
        ...
        split_if_true!(settings.l_upgrade_big && (vars.lure_ss.increased() || vars.lure_ic.changed() || vars.lure_ec.changed() || vars.lure_mr.changed()));
    ...
    if settings.act_split && vars.level.current != 0 && vars.level.current != 15 && vars.level.current != 22 && vars.level.current != 26 && vars.level.current != 29 && vars.level.current != 32 && vars.level.current != 33 && vars.level.current != 37 {
        split_if_true!(vars.act.changed())
    }
    
    let cur_b = &vars.event_bytes.current;
    let old_b = &vars.event_bytes.old;
    ...
    split_if_true!(settings.ship_transform && vars.level.current == 29 && vars.act.changed_from_to(&3, &6));
    split_if_true!(settings.back_from_past && vars.level.changed_from_to(&34, &33) && vars.act.current == 2 && vars.act.unchanged());
    split_if_true!(settings.enter_ss && vars.level.changed_from_to(&33, &26) && vars.act.changed_from_to(&0, &1));
    split_if_true!(settings.enter_mr && vars.level.changed_from_to(&26, &33) && vars.act.changed_from_to(&1, &0));
    
    // Gamma miscellaneous
    if vars.level.current == 33 && vars.act.current == 0 {
        split_if_true!(settings.death_wv_gamma && vars.adventure_data.changed() && vars.current_character.current == 6);
    }
    if vars.level.current == 33 && vars.act.current == 1 {
        split_if_true!(settings.death_mr_gamma && vars.lives.decreased() && vars.current_character.current == 6 && vars.adventure_data.current == 2);
    }
    ...
    if vars.level.current == 33 && vars.act.current == 1 {
        split_if_true!(settings.death_mr_knuckles && vars.lw_flag.current == 1 && vars.lives.decreased() && vars.current_character.current == 3);
    }
    ...
    if ((cur_b[0xA] & 0x10) >> 4) == 0 && vars.level.current == 26 && vars.act.changed_from_to(&3, &1) {
    ...
    split_if_true!(settings.enter_cave_super_sonic && vars.level.current == 33 && vars.act.changed_from_to(&0, &1));
    split_if_true!(settings.angel_island_super_sonic && vars.level.current == 33 && vars.act.current == 1 && vars.music.changed_to(&40));
    split_if_true!(settings.tikal_super_sonic && vars.level.changed_from_to(&34, &33));
    split_if_true!(settings.death_super_sonic && vars.level.current == 33 && vars.act.current == 1 && vars.lives.decreased());
    split_if_true!(settings.jungle_super_sonic && vars.level.current == 33 && vars.level.old != 0 && vars.act.changed_from_to(&0, &2));

    for i in 0..cur_b.len() {
        // Do not check during menus
        if vars.game_mode.current != 12 && (cur_b[i] != old_b[i]) {
            if vars.level.current < 15 { // The action stages
    ...
    if ((custom_vars.delay - vars.global_frame_count.current) as i64).abs() <= 2 && custom_vars.delay > 0 {
        custom_vars.delay = 0;
...
pub fn auto_splitter_update(vars: &GameStatePair, custom_vars: &mut CustomVars, settings: &Settings) {
    let raw_rta = custom_vars.run_start_time.map(|value| Instant::duration_since(&Instant::now(), value));
    ...
    if vars.in_prerendered_movie.current {
        custom_vars.total_cutscene_rta = custom_vars.total_cutscene_rta.add(current_rta - custom_vars.previous_rta);
    }
    
    if vars.game_mode.current == 1 || vars.game_mode.current == 12 || vars.game_mode.current == 18 || vars.game_mode.current == 20 || vars.game_status.current == 19 || vars.game_status.current == 16 {
        custom_vars.total_frame_count += vars.output_frame_count.current - vars.output_frame_count.old;
    }
    ...
    if vars.game_mode.current == 22 {
        ...
        if custom_vars.credits_counter == 0 {
            custom_vars.total_frame_count += match vars.current_character.current {
                0 => if vars.last_story.current == 0 { 16804 } else { 17360 },
                2 => 14657,
                3 => 16804,
                5 => 18950,
                6 => 17545,
...
pub fn auto_splitter_game_time(vars: &GameStatePair, custom_vars: &mut CustomVars, settings: &Settings) -> Duration {
    Duration::seconds_f32(custom_vars.total_frame_count as f32 / 60.0) + custom_vars.total_cutscene_rta
}

pub fn auto_splitter_is_loading(vars: &GameStatePair, custom_vars: &mut CustomVars, settings: &Settings) -> bool {
    true
}

pub fn auto_splitter_reset(vars: &GameStatePair, custom_vars: &mut CustomVars, settings: &Settings) -> bool {
    if settings.reset {
        return vars.game_mode.current == 12 && vars.current_character.old != 6 && vars.level_copy.current != 32;
    }
